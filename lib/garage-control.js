...
const deviceFilter = ["msg100"];
module.exports = function(RED) {
	'use strict';

	function GarageNode(node) {		
				...
				for(let i=0; i < _devices.length; i++){
					...
					switch(node.action){
						case 'state':
							...
							break;						
						case 'open':
							_devices[i].controlGarageDoor(0, 1, (err,res)=> {
								//console.log("res", res);
								//console.log("_devices[i]", _devices[i]);
								if(res){
									messages.push(
							...
							break;
						case 'close':
							_devices[i].controlGarageDoor(0, 0, (err,res)=> {
								//console.log("res", res);
								//console.log("_devices[i]", _devices[i]);
								if(res){
									messages.push(
	...
	RED.nodes.registerType('meross-garage', GarageNode);

	RED.httpAdmin.get(
		"/garage_devices/:nodeid",
		RED.auth.needsPermission("meross-garage.write"),
		function(req, res){
